...
locals {
  project_creator_sa = "${format("serviceAccount:%s", google_service_account.project_creator.email)}"
}
...
resource "google_storage_bucket_iam_member" "bucket_storage_objectAdmin" {
  bucket = "${google_project.bootstrap.project_id}"  # bucket name matches the terraform project name
